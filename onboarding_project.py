...
# event to function mappings

HANDLERS = {
    STATE.SETUP: Handler(),
    STATE.PICK_CHANCELLOR: Handler(),
    STATE.VOTE: Handler(),
    STATE.ELECTION_RESULTS: Handler(),
    STATE.PRESIDENT_DISCARD: Handler(),
    STATE.CHANCELLOR_DISCARD: Handler(),
    STATE.CHANCELLOR_VETOED: Handler(),
    STATE.ACTION: Handler(),
    STATE.END: Handler()
}

EVERYWHERE_FUNCTIONS = Handler()
...
def start():
    ...
    while True:
        ...
        if GAME_STATE in HANDLERS:
            state_handler = HANDLERS.get(GAME_STATE)
            if state_handler.can_handle(payload):
                state_handler.handle(payload)
            elif EVERYWHERE_FUNCTIONS.can_handle(payload):
                EVERYWHERE_FUNCTIONS.handle(payload)
            else:
...
@on_header(EVERYWHERE_FUNCTIONS, SHEPHERD_HEADERS.PLAYER_JOINED)
...
@on_header(HANDLERS[STATE.END], SHEPHERD_HEADERS.NEXT_STAGE)
def to_setup():
    """
...
@on_header(HANDLERS[STATE.SETUP], SHEPHERD_HEADERS.NEXT_STAGE)
def start_game():
    """
...
@on_header(HANDLERS[STATE.PICK_CHANCELLOR], SHEPHERD_HEADERS.CHANCELLOR_NOMINATION)
...
@on_header(HANDLERS[STATE.VOTE], SHEPHERD_HEADERS.PLAYER_VOTED)
...
@on_header(HANDLERS[STATE.PRESIDENT_DISCARD], SHEPHERD_HEADERS.PRESIDENT_DISCARDED)
...
@on_header(HANDLERS[STATE.CHANCELLOR_DISCARD], SHEPHERD_HEADERS.CHANCELLOR_VETOED)
...
@on_header(HANDLERS[STATE.CHANCELLOR_VETOED], SHEPHERD_HEADERS.PRESIDENT_VETO_ANSWER)
...
@on_header(HANDLERS[STATE.CHANCELLOR_DISCARD], SHEPHERD_HEADERS.CHANCELLOR_DISCARDED)
...
@on_header(HANDLERS[STATE.ACTION], SHEPHERD_HEADERS.INVESTIGATE_PLAYER)
...
@on_header(HANDLERS[STATE.ACTION], SHEPHERD_HEADERS.SPECIAL_ELECTION_PICK)
...
@on_header(HANDLERS[STATE.ACTION], SHEPHERD_HEADERS.END_POLICY_PEEK)
...
@on_header(HANDLERS[STATE.ACTION], SHEPHERD_HEADERS.END_INVESTIGATE_PLAYER)
...
@on_header(HANDLERS[STATE.ACTION], SHEPHERD_HEADERS.PERFORM_EXECUTION)
